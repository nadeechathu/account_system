          ...
          <div class="card">
            ...
            <div class="col-12 py-2 text-center px-5">
              ...
              <div class="card-body">
                  ...
                  <form class="form" action="{{ route('mobile.bulkReport')}}" method="get"
                  enctype="multipart/form-data">
                    ...
                    <div class="col-md-4 col-12 d-none">
                     ...
                     <div class="row">
                        <div class="col-6">
                            <button type="submit"
                            class="btn btn-primary mr-1 waves-effect waves-float waves-light w-100">Filter</button>
                        </div>
                        <div class="col-6">

                            <a href="{{route('mobile.bulkReport')}}" class="w-100"><button type="button" class="btn btn-outline-secondary waves-effect">Reset</button></a>
                        </div>
                     </div>
  ...
  <div class="row">
    <div class="col-12 px-3">
        <div class="row bg-primary py-1 text-white">
            ...
            <div class="col-2 d-none">Member Name</div>
            <div class="col-6 ">Member Code</div>
            <div class="col-1 d-none">Balance</div>
            <div class="col-6">Loan Amount</div>
            <div class="col-2 d-none">Collect date     <input type="date" class="form-control" id="abc" /></div>
            <div class="col-2 d-none">Action</div>
        </div>
        @php
    $totalLoanAmount = 0;
@endphp
...
@foreach ($loanCollections as $loan)
    ...
    @endphp
@endforeach
        <form action="{{route('mobile.bulkReport.update')}}" method="post" id="bulkForm">
            @csrf
            <div class="row text-dark border-bottom border-primary pt-1">
                ...
                <div class="col-1 pt-1 d-none">{{ $counter++ }}</div>
                <div class="col-2 pt-1 d-none">{{ $loanCollection->member->initial_name }}</div>
                <div class="col-6 pt-1">{{ $loanCollection->member->member_code }}</div>
                <div class="col-1 pt-1 d-none">
                  {{-- @php
                ...
                <div class="col-6">
                     <input type="tel" class="form-control py-1 mb-1" id="" onkeydown="return disableEnterKey(event)" name="loanAmounts[]"
                    value="{{ $loanCollection->loan_rental}}" onfocus="selectAllText(this)" onkeydown="handleKeyDown(event)">
                ...
                <div class="col-2 d-none"><input type="date" class="form-control py-1 mb-1 sum-input" name="collectDates[]" id="collect_date" value="{{ now()->format('Y-m-d') }}"></div>
...
<script>
    ...
    if (event.key === 'Enter' || event.keyCode === 13) {
        // Prevent the default action of "Next" button
        event.preventDefault();
